...
- Runtime Fabric initialization also skips when the app plist still contains an empty, whitespace-only, placeholder, or embedded placeholder API key.
- Testable Fabric API key validation keeps the runtime guard covered by XCTest cases for missing, blank, embedded placeholder fragments, case-insensitive placeholder values, and trimmed real values.
