...
- Runtime Fabric startup requires the original bundle value to be a whitespace-free, exact 40-hex API key, matching what the SDK consumes before Crashlytics can initialize.
- Testable Fabric API key validation keeps the runtime guard covered by XCTest cases for missing, blank, edge and embedded whitespace, Unicode control characters, embedded placeholder fragments, named placeholder fragments, case-insensitive placeholder values, exact lowercase and uppercase values, wrong lengths, and non-hex values.
